...
CaseSFML::CaseSFML(int posX, int posY, int largeur, int hauteur,const std::string & type)
{
    ...
    casesfml[0].texCoords = sf::Vector2f(posX,posY);
    casesfml[1].texCoords = sf::Vector2f(posX+largeur, posY);
    casesfml[2].texCoords = sf::Vector2f(posX+largeur, posY+hauteur);
    casesfml[3].texCoords = sf::Vector2f(posX, posY+hauteur);
